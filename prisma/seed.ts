...
import { PrismaClient, RoleType, SupplierType, StorageType } from '@prisma/client'
...
async function main() {
  ...
  const superAdminRole = await prisma.role.create({
    data: {
      ...
      permissions: DEFAULT_PERMISSIONS.SUPER_ADMIN as any
    }
  })
  ...
  const warehouseRole = await prisma.role.create({
    data: {
      name: 'Almacén',
      type: RoleType.WAREHOUSE,
      permissions: DEFAULT_PERMISSIONS.WAREHOUSE as any
    }
  })
